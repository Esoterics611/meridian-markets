...
- **Backend telemetry P1** (this session): `src/telemetry/` — config-gated `ITelemetry` swap seam + dependency-free Prometheus registry, `GET /metrics` + `GET /health[/ready]`, desk/feed/persist metrics pulled from `snapshot()` + the instrumented tick loop. Off by default. 143 suites / 942 tests.
