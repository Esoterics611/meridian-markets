...
| **Hyperliquid** | Perp-DEX (CLOB) | no-key (market data) | OHLCV + **L2 (20×20)** (funding not ingested yet) | **rebate −0.2 bps** | MM (top) + queue-aware fills + funding-carry + basis | `IReferenceBarSource` + `IL2BookSource` (+ MM `source`) | **S32/S33** — `HyperliquidClient` (candleSnapshot + `l2Snapshot`, both via POST), `hl-perps` scanner + MM presets. The maker-rebate CLOB the book needs; quotable since σ-normalization (S31). **L2 now feeds `LobReplayHarness`/`SimpleQueueModel`** → FIFO queue-aware fills (not fill-on-touch); `scripts/mm-l2-session.ts` polls the live book (S33, Journal #20). Funding ingest still open. Majors are tight (BTC TOB 0.15bps) → wide-spread edge is long-tail. |
