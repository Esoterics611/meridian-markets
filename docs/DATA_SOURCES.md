...
| **Hyperliquid SPOT** | Spot CLOB (HL-native) | no-key | **L2 (20×20)** via spotMeta pair resolution | **taker 7 / maker 4 bps — NO rebate on spot** (verified vs live docs 2026-07-16) | carry hedge leg for HL-only listings (HYPE, PURR) — single-venue pairs, no cross-venue basis, #92 collisions impossible | `HyperliquidSpotClient` | **#100** — `spotMeta` (token → "@N" pair id, cached) + `l2Book` through the perp parser; priced as `venueFeeFor('hyperliquid-spot')` — never borrow the perp rebate row for a spot leg. |
