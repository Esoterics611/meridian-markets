...
7. **Naive spread MM has no edge — the fix is fair value + cadence, not a wider spread (2026-06-05, the decisive MM result).** A 6h/20-perp L2 harvest showed `spread − adverse` is **negative at every spread width on every liquid coin** with inventory clamped — adverse selection eats the whole half-spread, and *widening does not help* because adverse is a **fair-value problem (stale-mid selection), not a width problem**. Two fixes, both measured on the saved tapes: (a) quote around the **book-imbalance micro-price** instead of the mid → **−21% adverse** desk-wide; (b) **re-quote sub-second** instead of every 10–18s → the dominant lever, flipping the desk's `spread − adverse` from **−$1,020 (18s) → +$133 (sub-second)**, positive on all 5 liquid coins, ETH/DOGE net-positive at low drawdown. A **Binance→HL cross-venue** fusion was *measured* and is a **no-op** — HL price-discovers on its own (the operator's call, confirmed: HL is a lead venue, not a follower). The only remaining loss is **inventory carry** on trending coins → the **directional / "axed" maker** (`mm-directional-glft`, rests at a target inventory `q*=bias·maxLots`) turns it into a chosen bet, but **only with an OOS-validated bias** (a blind bias measurably loses). **Decisive lesson:** DEX market-making is a real business, but the edge is **fair-value prediction + speed**, not spreads. **Honest caveats:** one 8h window; 88% of sub-second steps used a flow *estimate* (sparse WS prints), so the qualitative flip is robust but the exact number is provisional — a clean read needs event-driven WS flow + a modelled cancel/replace latency. Trail: [QUANT_JOURNAL.md](QUANT_JOURNAL.md) #27–#33; design: [FAIR_VALUE_AND_THESIS_DESIGN.md](FAIR_VALUE_AND_THESIS_DESIGN.md), [DIRECTIONAL_MM_STRATEGY.md](DIRECTIONAL_MM_STRATEGY.md).
