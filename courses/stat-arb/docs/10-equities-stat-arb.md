...
1. **More history.** Backfill decades of daily bars. This is the cleanest fix for *trade count* — and the engine now has a free long-history source (`YahooDailyClient`, split+dividend-adjusted, `OOS_SOURCE=yahoo`). **But beware what it reveals (Journal #13):** the pooled per-trade Sharpe *rose* with window length — 0.06 (5yr) → 0.09 (10yr) → 0.15 (24yr) — and the gate flipped to "PASS" (PSR ~100% at n≈1,800). A rising Sharpe with more history is the **signature of survivorship bias**, not a stable edge: a 24-year backtest on *today's* index members silently drops the firms that died (the 2008 bank casualties), whose pairs would have lost. So more history solved the trade-count problem and confirmed the edge is *positive*, but it exposed the real binding blocker — **survivorship** (§10.5). A `PASS` on PSR with huge n is not a deployable green light; a 0.09–0.15 per-trade Sharpe book draws down often, and the level is an upper bound until a point-in-time universe removes the survivor inflation.
