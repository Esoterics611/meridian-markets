...
!!! note "Procedure — capture a tape and replay"
    ...
    ```
    `mm-l2-session.ts` builds the tape from real depth and gates touches off each candle's *real* high/low; only per-interval aggressive *volume* is estimated, and that estimate is stated honestly in the output. The fee sweep prints structural / rebate / cost columns and the drawdown.
...
- **The per-pool *verdict* needs a long capture.** The tuning *machinery* is proven (unit tests pin the differentiation, ranking, fee model, and tape round-trip), but at 5-second-prorated volume on top-of-book BTC/ETH every combo fills 0 — the queue is thick and the estimated flow thin. Whether HL's −0.2bps rebate makes *any* calibration **net-positive on honest fills** is still an open question, pending a 60s-poll multi-hour capture.
- **Aggressor flow is estimated, not observed.** The harness infers per-interval aggressive *volume* from candles. The real fix is HL's `trades` feed / WebSocket for true per-trade aggressor data — that replaces the estimate and is the next fidelity step.
- **Funding carry is not yet in the cost model.** HL perps pay/charge funding; the carry leg ([§8.2](#82-the-default-venue-hyperliquid)) is not yet priced. Until it is, treat a held perp inventory as carrying an unmodelled cost and keep inventory tight.

None of these is a reason not to run the stack — they are the difference between "this works on honest fills" (where we are) and "we know the per-pool number cold" (where the long capture takes us).
