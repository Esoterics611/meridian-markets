...
- **Funding carry — the rate is now sourced and priced; accrual on the live book is the remaining step.** HL perps pay/charge funding hourly, and that flow accrues on whatever inventory the MM book holds. The data layer now exists: `HyperliquidFundingClient` (`IFundingRateSource` over the HL `info` POST — hourly `fundingHistory` + `metaAndAssetCtxs`) and `staticCarry` (generalised to a venue `periodsPerYear`, HL = 8760), with `scripts/funding-carry-research.ts FC_SOURCE=hyperliquid` pricing the stream (a recent 15-day read: ETH ≈ 8.1%/yr, BTC ≈ 4.5%/yr, funding persistently positive). What remains is **accruing that rate on the queue-aware book's held inventory** so the session equity curve includes funding, not just spread − adverse − fees. Until that lands, treat a held perp inventory as carrying an unmodelled (here, *favourable*) funding flow and keep inventory tight.
