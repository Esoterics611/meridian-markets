...
- **Run it:** stat-arb: `FEED_SOURCE=binance EXECUTION_MODE=paper MOCK_TRADING_ENABLED=false npm run start:dev` → `/demo` + `/api/stat-arb/live/*`; MM books on the same process at `/api/market-making/*`. Quant research loop: `npx ts-node -r tsconfig-paths/register scripts/quant-research.ts`. Real-money is the `EXECUTION_MODE=live` + `LIVE_TRADING_ARMED=true` engineering decision — no business/KYB gate in the engine.
- **Done (S22 — P0.2 real-history walk-forward OOS gate + desk-scale lots):** the `walkForward` harness is now slice/train-aware (`venueFactory(barsA,barsB)` so a replay venue prices each window; `strategyFactory(trainCtx)` so β is fit on train and applied OOS). New `POST /api/market-data/walk-forward` runs the active pair over **real Binance history** with a true train/test split — **β re-fit per train window** (Engle-Granger), net of fee+spread+impact, returning `oos.avgTestSharpe`/`positiveWindowShare`/**`sharpeDegradation`** (train→test gap) + **β per window** (β drift/sign-flips visible). Surfaced in Research as **↻ Walk-forward (real OOS — active pair)** alongside the synthetic tools (relabelled). **Desk-scale lots:** killed single-dollar toy moves — synthetic research demos + `/backtest` + `/walk-forward` default to **$100k/leg** (the live desk lot still drives the UI). Paste-ready next-session prompt at [docs/QUANT_SESSION_PROMPT.md](docs/QUANT_SESSION_PROMPT.md). 111 suites / 722 tests.
- **Next (P0 frontier — gate before trusting any backtest, per [docs/PRODUCTION_READINESS.md](docs/PRODUCTION_READINESS.md)):** ✅ **OOS / walk-forward on REAL history is DONE (S22)** — *but still needs running on the deploy candidates to record numbers* (Journal next-action #1). Remaining P0: **multiple-testing correction** (deflated Sharpe + purged k-fold — we scan ~80–90 pairs/class); **borrow/funding cost on the short leg**; **more history + point-in-time universe** (avoid survivorship). Then **P1 (real capital):** risk-parity allocator wired into the live path, maker/limit execution (reuse `src/market-making/`), real-venue adapter + reconciliation, restart-safe live books. Also still open: **cross-source pairing** (USD/ILS Pyth × USDC/NIS Bit2C basis), funding-carry + cross-sectional baskets (course §8), L2 ingest → honest `LobReplayHarness`, WebSocket feed, course gaps (Johansen).
