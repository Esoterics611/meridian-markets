...
Full per-session log in [docs/SESSION_HISTORY.md](docs/SESSION_HISTORY.md). Current state (as of 2026-05-31):
...
- **Next:** **cross-asset opportunity scanner** — net-edge-after-fees ranking sweep over all presets + the TESSERA stablecoin/FX/ILS universe, surfacing the few pairs that clear the fee gate (reuses `discovery/pair-discovery` + `signal-decay` + `signal/fee-gate`); **MM spread-capture screener** (rank instruments by realised-spread − adverse − inventory-risk + rebate); **reference-data adapters** for the basis trades (OANDA/Pyth FX, DefiLlama peg, Bit2C `USDC/NIS`) per [TESSERA]; **L2 ingest** to turn `SimpleQueueModel` into the honest `LobReplayHarness`. Also still open: funding-carry + cross-sectional baskets (course §8), budget allocator, Track-B real-venue adapter (git `stash@{0}`), WebSocket feed, course gaps (Johansen, purged k-fold, deflated-Sharpe — now doubly relevant given the scanner's multiple-testing risk).
