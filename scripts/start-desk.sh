...
MM_FAST_SYMBOLS="${MM_FAST_SYMBOLS:-SOL,ADA,DOGE,SUI,FARTCOIN,kPEPE,AAVE,PUMP,CRV,TAO,XRP,BNB,HYPE,ZEC,NEAR,WLD,VVV,TRUMP,XPL,LIT,TON,MEGA,ENA,ONDO,XMR}" \
...
MM_HEDGE_BETA_MAP="${MM_HEDGE_BETA_MAP:-SOL|ETH|1.02,ADA|ETH|1.05,DOGE|ETH|0.94,SUI|ETH|1.29,FARTCOIN|ETH|1.50,kPEPE|ETH|1.20,AAVE|ETH|1.11,PUMP|ETH|1.51,CRV|ETH|1.14,TAO|BTC|1.69,XRP|BTC|1.15,BNB|BTC|0.92}" \
MM_HEDGE_MIN_HOLD_MS="${MM_HEDGE_MIN_HOLD_MS:-30000}" \
MM_HEDGE_FLIP_COOLDOWN_MS="${MM_HEDGE_FLIP_COOLDOWN_MS:-300000}" \
MM_HEDGE_FLOW_FREEZE_THETA="${MM_HEDGE_FLOW_FREEZE_THETA:-0.25}" \
MM_HEDGE_BASIS_GATE="${MM_HEDGE_BASIS_GATE:-FARTCOIN:flatten,kPEPE:flatten,ADA:flatten}" \
MM_HEDGE_BAND_MAP="${MM_HEDGE_BAND_MAP:-}" \
MM_REQUOTE_MIN_BPS="${MM_REQUOTE_MIN_BPS:-0}" \
MM_REQUOTE_DWELL_MS="${MM_REQUOTE_DWELL_MS:-400}" \
MM_REQUOTE_URGENT_BPS="${MM_REQUOTE_URGENT_BPS:-4}" \
MM_CONC_SOFT="${MM_CONC_SOFT:-0.5}" \
MM_CONC_HARD="${MM_CONC_HARD:-0.85}" \
MM_CONC_SKEW_GAIN="${MM_CONC_SKEW_GAIN:-2}" \
