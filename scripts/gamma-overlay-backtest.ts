 ...
 * decision is REALISED vol vs IMPLIED vol — so this script measures both on live data and prints the
 * verdict for a given desk short-gamma bleed.
 *
 *   realised vol  ← Hyperliquid BTC 1m candles over the lookback window
 *   implied vol   ← Deribit nearest-expiry ATM BTC mark_iv
 *   verdict       ← gammaOverlay(): recover fraction = 1 − iv²/rv², net = bleed·fraction − cost
 *
 * Run: npx ts-node -r tsconfig-paths/register scripts/gamma-overlay-backtest.ts [hours] [bleedUsd] [costUsd]
 */
import { realizedVolAnnualized, gammaOverlay } from '../src/market-making/hedge/gamma-overlay';
...
async function hlBtcCloses(hours: number): Promise<number[]> {
  const end = Date.now();
  const start = end - hours * 3_600_000;
  const body = JSON.stringify({ type: 'candleSnapshot', req: { coin: 'BTC', interval: '1m', startTime: start, endTime: end } });
...
async function deribitAtmIv(): Promise<{ iv: number; instrument: string; index: number }> {
  ...
  // instrument_name = BTC-<DDMMMYY>-<strike>-<C|P>; parse expiry + strike.
...
async function main() {
  ...
  const bleedUsd = Number(process.argv[3] ?? 2_345); // #41 desk adverse-selection total (short-gamma proxy)
  const costUsd = Number(process.argv[4] ?? 0);

  const [closes, atm] = await Promise.all([hlBtcCloses(hours), deribitAtmIv()]);
  const realizedVol = realizedVolAnnualized(closes, 365 * 24 * 60);
  ...
  console.log(`overlay recovers                $${r.recoveredUsd.toFixed(0)}`);
  console.log(`overlay NET (− cost)            $${r.netUsd.toFixed(0)}`);
